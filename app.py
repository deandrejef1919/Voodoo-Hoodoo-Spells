# app.py - Voodoo & Hoodoo Spells (MASTER BLOCK)
...
if "app_theme" not in st.session_state:
    st.session_state["app_theme"] = "day"  # "day" or "night"

# ------------------------------------------------------
# DAY & NIGHT CSS (apply via apply_theme_css)
# ------------------------------------------------------
DAY_CSS = """
<style>
body, .stApp {
...
.vh-logo { font-size: 3.4rem; text-shadow: 0 0 8px rgba(244,67,54,0.9), 0 0 14px rgba(0,0,0,1.0); animation: mojo-heartbeat 1.6s ease-in-out infinite; }
.vh-title { font-size: 2.4rem; font-weight: 800; letter-spacing: 0.18em; text-transform: uppercase; color: #f44336; text-shadow: 0 0 10px rgba(244,67,54,0.9), 0 0 20px rgba(0,0,0,1.0); }
...
.sidebar-logo { text-align:center; font-size: 1rem; font-weight: 700; margin: 0.75rem 0 1.2rem 0; letter-spacing: 0.16em; text-transform: uppercase; padding: 0.6rem 0.4rem; border-radius: 14px;
    background: radial-gradient(circle at 30% 0%, #3b0000 0%, #050505 55%, #000 100%); border: 1px solid rgba(244,67,54,0.7);
    box-shadow: 0 0 10px rgba(244,67,54,0.6), 0 0 18px rgba(0,0,0,0.9), inset 0 0 6px rgba(0,0,0,0.7);
}

/* Mojo tiles (separate pulse for each) */
.mojo-video-glow { border-radius: 16px; padding: 0.45rem; background: radial-gradient(circle at 50% 0%, #330000 0, #120000 55%, #000000 100%); box-shadow: 0 0 4px rgba(255, 0, 0, 0.7); animation: mojo-video-pulse 1.6s ease-in-out infinite; }
.mojo-bag-glow { border-radius: 16px; padding: 0.45rem; background: radial-gradient(circle at 50% 0%, #2a1b00 0, #120b00 55%, #000000 100%); box-shadow: 0 0 4px rgba(255, 215, 0, 0.7); animation: mojo-bag-pulse 1.6s ease-in-out infinite; }
...
@keyframes mojo-heartbeat {
    ...
    20% { box-shadow: 0 0 25px 8px rgba(255, 0, 0, 0.85); }
    35% { box-shadow: 0 0 10px 3px rgba(255, 0, 0, 0.45); }
    55% { box-shadow: 0 0 20px 6px rgba(255, 0, 0, 0.75); }
    100% { box-shadow: 0 0 0 0 rgba(255, 0, 0, 0.0); }
}

/* Sharper pulses */
@keyframes mojo-video-pulse { 0% { box-shadow: 0 0 4px rgba(255,0,0,0.6); } 50% { box-shadow: 0 0 14px rgba(255,0,0,1.0); } 100% { box-shadow: 0 0 4px rgba(255,0,0,0.6); } }
@keyframes mojo-bag-pulse   { 0% { box-shadow: 0 0 4px rgba(255,215,0,0.6); } 50% { box-shadow: 0 0 14px rgba(255,215,0,1.0); } 100% { box-shadow: 0 0 4px rgba(255,215,0,0.6); } }

/* Incense smoke */
.incense-container { text-align:center; margin-top:0.5rem; }
.incense-burner { font-size:32px; }
.smoke { width:4px; height:60px; margin:0 auto; background: linear-gradient(to top, rgba(255,255,255,0.0), rgba(255,255,255,0.45)); border-radius:999px; animation: smoke-rise 4.2s ease-in-out infinite; }
@keyframes smoke-rise { 0% { opacity:0.0; transform: translateY(20px);} 20% { opacity:0.5; transform: translateY(0px);} 80% { opacity:0.5; transform: translateY(-24px);} 100% { opacity:0.0; transform: translateY(-40px);} }

/* Cards */
.vh-card { border-radius:16px; border:1px solid rgba(244,67,54,0.75); padding:1.2rem 1.45rem; margin-bottom:1.1rem; background: radial-gradient(circle at top, #191313 0%, #090808 55%, #010101 100%); box-shadow: 0 0 12px rgba(244,67,54,0.45), 0 0 26px rgba(0,0,0,0.98); }
.vh-card h3 { margin-top:0; }
...
@keyframes shieldGlow {
  0% { box-shadow: 0 0 10px rgba(244,67,54,0.9), 0 0 18px rgba(76,175,80,0.7); }
  100% { box-shadow: 0 0 18px rgba(244,67,54,1.0), 0 0 28px rgba(76,175,80,0.9); }
}
div.stButton > button { border-radius: 999px / 70px; border-width: 2px; border-style: solid; border-color: #1b5e20; padding: 0.45rem 1.9rem; font-weight:700; text-transform:uppercase; letter-spacing:0.09em; font-family:"Times New Roman", Times, serif; font-size:15px; background-image: linear-gradient(135deg, #f44336 0%, #000000 40%, #1b5e20 100%), radial-gradient(circle at 16% 0%, rgba(255,255,255,0.25) 0%, transparent 60%); background-blend-mode: overlay; color:#fff7ec; animation: shieldGlow 2.4s ease-in-out infinite alternate; transform: translateY(0); }
div.stButton > button::before { content: "⚔️"; margin-right: 0.35rem; }
div.stButton > button:hover { border-color: #f44336; transform: translateY(-2px); }

/* Footer */
.vh-footer { text-align:center; font-size:0.9rem; color:#c0bbb2; margin-top:2.8rem; padding-top:0.9rem; border-top:1px solid rgba(76,175,80,0.7); opacity:0.95; }
</style>
"""
...
.vh-header { text-align:center; padding: 0.75rem 0 0.25rem 0; }
.vh-logo { font-size: 3.4rem; text-shadow: 0 0 14px rgba(156,39,176,0.9); animation: mojo-heartbeat 1.6s ease-in-out infinite; }
.vh-title { font-size: 2.4rem; font-weight: 800; letter-spacing: 0.18em; color: #ff4081; text-shadow: 0 0 12px rgba(255,64,129,0.9); }
.sidebar-logo { text-align:center; font-size:1rem; font-weight:700; margin:0.75rem 0 1.2rem 0; padding:0.6rem 0.4rem; border-radius:14px; background: radial-gradient(circle at 30% 0%, #25002c 0%, #050208 55%); border:1px solid rgba(156,39,176,0.8); box-shadow: 0 0 12px rgba(156,39,176,0.7);}
.mojo-video-glow { border-radius:16px; padding:0.45rem; background: linear-gradient(90deg, rgba(40,6,30,0.8), rgba(5,5,10,0.95)); box-shadow: 0 0 10px rgba(156,39,176,0.8); animation: mojo-video-pulse 1.6s ease-in-out infinite; }
.mojo-bag-glow { border-radius:16px; padding:0.45rem; background: linear-gradient(90deg, rgba(30,20,0,0.9), rgba(5,3,0,0.95)); box-shadow: 0 0 10px rgba(255,193,7,0.8); animation: mojo-bag-pulse 1.6s ease-in-out infinite; }
.incense-container { text-align:center; margin-top:0.5rem; } .smoke { animation: smoke-rise 4.2s ease-in-out infinite; }
</style>
"""

def apply_theme_css():
    theme = st.session_state.get("app_theme", "day")
    st.markdown(DAY_CSS if theme == "day" else NIGHT_CSS, unsafe_allow_html=True)

apply_theme_css()

# ------------------------------------------------------
# LWA DATA (19) – includes Baron Semedi using the exact key you supplied
# ------------------------------------------------------
LOA_GALLERY_DATA = [
    {
        "name": "Papa Legba",
    ...
    {
        "name": "Baron Semedi",
        "image_key": "LWA_BARON_SEMEDI_URL",  # exact key you provided
        "description": "Baron Semedi (Baron Samedi) is a powerful Gede lwa associated with death, the ancestors, and the boundary between life and afterlife.",
        "attributes": [
            "🕯️ Offerings: rum with hot pepper, cigars, black/white flowers, peanuts",
            "💀 Domains: death, graves, rebirth, ancestor protection",
            "📆 Day: Saturday • 🎨 Colors: black, purple, white",
            "🔔 Syncretized saint: St. Gerard / St. Martin (varies by locale)",
            "⚠️ Caution: approach with respect; humor is accepted but never mockery.",
        ],
    },
    {
        "name": "Ogoun Badagri",
...
def page_settings():
    ...
    st.markdown("### Theme")
    current_theme = st.session_state.get("app_theme", "day")
    choice = st.radio(
        "Choose app theme",
        ["Current (Red/Green Day)", "Dark Ritual Night"],
        index=0 if current_theme == "day" else 1,
    )

    if choice == "Current (Red/Green Day)":
        st.session_state["app_theme"] = "day"
    else:
